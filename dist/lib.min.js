...
b.document,isReady:!1,isDOMReady:!1,ready:function(f,b){f===e?(a.isReady=!0,a.event.dispatch(a.document,"libReady"),a.event.remove(a.document,"libReady")):"function"===typeof f&&(!a.isReady&&!b?a.event.add(a.document,"libReady",a.bind(f,a.window)):!a.isDOMReady&&b?a.event.add(a.document,"DOMReady",a.bind(f,a.window)):f())},extend:function(a){for(var e,b=0,d=arguments.length;++b<d;)for(e in arguments[b])arguments[b].hasOwnProperty(e)&&(a[e]=arguments[b][e]);return a},bind:function(a,e){return"bind"in
a?a.bind(e):function(){return a.apply(e,arguments)}},guid:function(f){a.guid.id||(a.guid.id=1);if(!f)return++a.guid.id;f.__guid||(f.__guid=a.guid.id++);return f.__guid}};b.lib=a;b.log=d})(window);
...
(function(b){b.object={keys:function(e){var b;if("keys"in Object)return Object.keys(e);b=[];for(var a in e)e.hasOwnProperty(a)&&b.push(a);return b},createFromTemplate:function(e,b){var b=b?b:{},a=e.split(".");(function f(a,b){a[b[0]]||(a[b[0]]={});1<b.length&&f(a[b[0]],b.splice(1))})(b,a);return b},subtract:function(b,d){var a={},c;for(c in b)"undefined"===typeof d[c]&&(a[c]=b[c]);return a}}})(lib);
...
f,d,g,e;i<c;)if(b=a.charAt(i),-1<"\t\r\n ".indexOf(b))i++;else{if(-1<"{}[]:,".indexOf(b))return i++,b;if('"'==b){f="@";for(i++;i<c;)if(b=a.charAt(i)," ">b)m();else if("\\"==b)if(b=a.charAt(++i),-1<'\\"/btnfr'.indexOf(b))f+=t[b],i++;else if("u"==b){d=++i;for(g=i+4;i<g;i++)b=a.charAt(i),"0"<=b&&"9">=b||("a"<=b&&"f">=b||"A"<=b&&"F">=b)||m();f+=s("0x"+a.slice(d,i))}else m();else{if('"'==b)break;f+=b;i++}if('"'==a.charAt(i))return i++,f}else{d=i;"-"==b&&(e=!0,b=a.charAt(++i));if("0"<=b&&"9">=b){for("0"==
b&&(b=a.charAt(i+1),"0"<=b&&"9">=b)&&m();i<c&&(b=a.charAt(i),"0"<=b&&"9">=b);i++);if("."==a.charAt(i)){for(g=++i;g<c&&(b=a.charAt(g),"0"<=b&&"9">=b);g++);g==i&&m();i=g}b=a.charAt(i);if("e"==b||"E"==b){b=a.charAt(++i);("+"==b||"-"==b)&&i++;for(g=i;g<c&&(b=a.charAt(g),"0"<=b&&"9">=b);g++);g==i&&m();i=g}return+a.slice(d,i)}e&&m();if("true"==a.slice(i,i+4))return i+=4,!0;if("false"==a.slice(i,i+5))return i+=5,!1;if("null"==a.slice(i,i+4))return i+=4,null}m()}return"$"},u=function(a){var c,b;"$"==a&&m();
